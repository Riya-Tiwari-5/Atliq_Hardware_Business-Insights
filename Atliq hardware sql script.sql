...
Round((c2.unique_product_2021-c1.unique_product_2020)*100/c1.unique_product_2020,2)
AS percentage_chg
...
SELECT 
    c.customer_code,
    customer,
    ROUND(AVG(pre_invoice_discount_pct) * 100, 4) AS average_discount_percentage
FROM
    dim_customer c
        JOIN
...
WITH CTE1 as
     ...
     from fact_sales_monthly)
select case
      when period in (9,10,11) then 'Q1'
      when period in (12,1,2) then 'Q2'
      when period in (3,4,5) then 'Q3'
      when period in (6,7,8) then 'Q4' end as quarter,
...
WITH CTE1
     AS (SELECT p.division,s.product_code,p.product,
     ...
     CTE2
     AS (SELECT division,product_code,product,total_sold_quantity,
                RANK()
                  OVER(
